...
The first conjunct is intuitively $\true$, as the maximal run of an array of $\LEN = 1$ is itself.

The \textbf{LHS} of the implication in the second conjunct is $true$ only when $p = 0$ and $q = 1$. The \textbf{RHS} is then $(1 - 0) \leqslant (1 - 0)$. Thus, the implication is $\true$ for these values of $p$ and $q$. All other values of $p$ and $q$ cause the implication to be $\true$. Thus, the second conjunct is $\true$. Thus, the entailment holds as
