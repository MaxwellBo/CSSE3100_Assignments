...
where $guard$ is a function that takes $i$, $j$, $k$ as implicit parameters.
...
\begin{eqnarray*}
(A_i > B_j) \lor (B_j > C_k) \lor (C_k > A_i) \lor (A_i = B_j = C_k) & \equiv & ((A_i > B_j) \lor (B_j > C_k) \lor (C_k > A_i) \lor (A_i = B_j = C_k))\\
