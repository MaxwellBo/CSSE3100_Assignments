...
\form{(\GUARDUNION) ~\land~ (\BOUNDEDNONINCLUSIVE{i}{j}{k})}
\ENDDERIVE
...
\begin{eqnarray*}
guard[i, j, k, r \backslash 0, A.len, 0, 0] & \equiv & \GUARD[i, j, k, r \backslash 0, A.len, 0, 0]\\
    & \equiv & A.len \neq A.len\, \lor\, 0 \neq B.len\, \lor\, 0 \neq C.len\\
...
\begin{eqnarray*}
&& (G_{union}(i, j, k) ~\land~ implicit\_constraints(i, j, k))[i, j, k, l \backslash, 0, A.len, 0, 0]\\
& \equiv &\\
...
\begin{eqnarray*}
(inv\, \land\, guard)[i, j, k, r \backslash 0, A.len, 0, 0] &\not\implies& (G_{union}(i, j, k)\\
&& \land~ implicit\_constraints(i, j, k))[i, j, k, r \backslash 0, A.len, 0, 0]\\
inv[i, j, k, r \backslash 0, A.len, 0, 0]\, \land\, guard[i, j, k, r \backslash 0, A.len, 0, 0] &\not\implies& \false\\
\true\, \land\, \true &\not\implies& \false\\
\true &\not\implies& \false\\
\end{eqnarray*}
